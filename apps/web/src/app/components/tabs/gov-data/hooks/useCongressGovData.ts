...
export function useCongressGovData(): UseCongressGovDataReturn {
  ...
  const fetchBills = useCallback(async () => {
    ...
    try {
      ...
      const parsed = parseInvokeResult<CongressBillsResponse>(result);
      if (!parsed.success) {
  ...
  const fetchBillDetail = useCallback(async (billUrl: string) => {
    ...
    try {
      ...
      const parsed = parseInvokeResult<CongressBillDetail>(result);
      if (!parsed.success) {
  ...
  const fetchSummary = useCallback(async () => {
    ...
    try {
      ...
      const parsed = parseInvokeResult<CongressBillSummary>(result);
      if (!parsed.success) {
