...
export function useCanadaGovData(): UseCanadaGovDataReturn {
  ...
  const fetchPublishers = useCallback(async () => {
    ...
    try {
      ...
      const parsed = parseInvokeResult<CanadaGovResponse<CanadaPublisher[]>>(result);
      if (parsed.error) {
  ...
  const fetchDatasets = useCallback(async (publisherId: string) => {
    ...
    try {
      ...
      const parsed = parseInvokeResult<CanadaGovResponse<CanadaDataset[]>>(result);
      if (parsed.error) {
        setError(parsed.error);
        setDatasets([]);
  ...
  const fetchResources = useCallback(async (datasetId: string) => {
    ...
    try {
      ...
      const parsed = parseInvokeResult<CanadaGovResponse<CanadaResource[]>>(result);
      if (parsed.error) {
        setError(parsed.error);
        setResources([]);
      } else {
