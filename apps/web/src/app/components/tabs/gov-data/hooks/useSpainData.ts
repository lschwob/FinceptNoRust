...
import { parseInvokeResult } from '@/services/core/invokeUtils';
import type {
  SpainDataView,
  SpainCatalogue,
  SpainDataset,
...
export function useSpainData(): UseSpainDataReturn {
  ...
  const fetchCatalogues = useCallback(async () => {
    ...
    try {
      ...
      const parsed = parseInvokeResult<SpainDataResponse<{ items: SpainCatalogue[] }>>(result);
      if (parsed.error) {
  ...
  const fetchDatasets = useCallback(async (publisherId?: string) => {
    ...
    try {
      ...
      const parsed = parseInvokeResult<SpainDataResponse<{ items: SpainDataset[] }>>(result);
      if (parsed.error) {
        setError(parsed.error);
        setDatasets([]);
      } else {
  ...
  const fetchResources = useCallback(async (datasetId: string) => {
    ...
    try {
      ...
      const parsed = parseInvokeResult<SpainDataResponse<SpainResource[]>>(result);
      if (parsed.error) {
        setError(parsed.error);
        setResources([]);
      } else {
