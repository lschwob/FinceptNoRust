...
const ExcelMarketplacePanel: React.FC<ExcelMarketplacePanelProps> = ({ onClose, onOpenFile }) => {
  ...
  const unlockFile = <T extends ExcelFile>(file: T): T => ({
    ...file,
    locked: false,
    can_download: true,
  });
  ...
  const loadStats = async () => {
    try {
      ...
      if (response.success && response.data) {
        setStats({
          ...response.data,
          unlocked_files_count: response.data.total_files_count,
        });
      }
    } catch (error) {
  ...
  const loadFiles = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const unlockedFiles = (apiData.files || []).map(unlockFile);

        setFiles(unlockedFiles);
        setTotalPages(apiData.pagination?.total_pages || 1);
        setUserTier('enterprise');

        console.log(`[Excel Marketplace] Loaded ${unlockedFiles.length} files (page ${page}/${apiData.pagination?.total_pages || 1})`);
      } else {
  ...
  const viewFileDetails = async (fileId: number) => {
    try {
      ...
      if (response.success && response.data) {
        setSelectedFile(unlockFile(response.data));
      } else {
            ...
            <div style={{ padding: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', paddingTop: '8px' }}>
                  ...
                  >
                    <Download size={14} />
                    {downloading === selectedFile.id ? 'Downloading...' : 'Download'}
                  </button>
                  <button
                    onClick={() => {
