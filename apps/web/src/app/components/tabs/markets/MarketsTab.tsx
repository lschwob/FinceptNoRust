...
const MarketsTab: React.FC = () => {
  ...
  // Initialize: try SQLite, then always load preferences (fallback to defaults so market panels can fetch)
  useEffect(() => {
    let mounted = true;
    ...
    const init = async () => {
      try {
        ...
        if (!mounted) return;
        setInitError(err instanceof Error ? err.message : 'Storage init failed');
      }
      try {
        ...
        if (!mounted) return;
        const fallback = await tickerStorage.loadPreferences();
        dispatch({ type: 'SET_PREFERENCES', payload: fallback });
      }
      if (!mounted) return;
