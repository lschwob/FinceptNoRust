...
import {
  Activity, Search, RefreshCw, Wallet, Bot, CheckCircle, BarChart3, Star,
} from 'lucide-react';
...
import Polymarket5mStatsView from './Polymarket5mStatsView';
import Polymarket5mBacktestView from './Polymarket5mBacktestView';
...
const PolymarketTab: React.FC = () => {
  ...
  const [upDownRefreshTick, setUpDownRefreshTick] = useState(0);
  const [statsRefreshTick, setStatsRefreshTick] = useState(0);
  const [backtestRefreshTick, setBacktestRefreshTick] = useState(0);
  const pageSize = 20;
  ...
  // ── Watchlist state ───────────────────────────────────────────────────────────
  const [watchlistEntries, setWatchlistEntries] = useState<PolymarketWatchlistEntry[]>([]);
  ...
  useEffect(() => {
    ...
    else if (activeView === 'watchlist') loadWatchlist();
  ...
  const loadWatchlist = async () => {
    ...
    setWatchlistEntries(list);
  };

  const handleWatchlistSelect = async (entry: PolymarketWatchlistEntry) => {
    if (entry.slug) {
      try {
        const market = await polymarketApiService.getMarketBySlug(entry.slug);
        await loadMarketDetails(market);
      } catch {
        const minimal: PolymarketMarket = {
          id: entry.marketId,
          question: entry.question,
          outcomePrices: entry.outcomePrices ?? ['0', '0'],
          outcomes: '["Yes","No"]',
        ...
        await loadMarketDetails(minimal);
      }
    } else {
      const minimal: PolymarketMarket = {
        id: entry.marketId,
  ...
  const handleWatchlistRemove = async (marketId: string) => {
    await polymarketWatchlistService.removeFromWatchlist(marketId);
    await loadWatchlist();
    if (selectedMarket?.id === marketId) setSelectedMarket(null);
  };
  ...
  const handleRefresh = () => {
    ...
    else if (activeView === 'updown') setUpDownRefreshTick(v => v + 1);
    else if (activeView === 'stats-5m') setStatsRefreshTick(v => v + 1);
    else if (activeView === 'backtest-5m') setBacktestRefreshTick(v => v + 1);
  ...
  const activeViewLabel = activeView === 'updown'
    ? 'UP/DOWN'
    : activeView === 'stats-5m'
      ? '5M STATS'
      : activeView === 'backtest-5m'
        ? '5M BACKTEST'
    : activeView === 'portfolio-tracker'
      ? 'TRACKER'
  ...
  return (
    <div style={{ height: '100%', backgroundColor: C.bg, color: C.white, fontFamily: C.font, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ borderBottom: `2px solid ${C.orange}`, backgroundColor: C.bg, flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', padding: '0 12px', gap: 1 }}>
          {[
            ...
            { key: 'watchlist', label: 'WATCHLIST', icon: <Star size={10} style={{ marginRight: 3 }} /> },
            { key: 'updown',    label: 'UP/DOWN',   icon: <Activity size={10} style={{ marginRight: 3 }} /> },
            { key: 'stats-5m',  label: '5M STATS',  icon: <BarChart3 size={10} style={{ marginRight: 3 }} /> },
            { key: 'backtest-5m', label: '5M BACKTEST', icon: <BarChart3 size={10} style={{ marginRight: 3 }} /> },
        ...
        {!isFullscreen && !isBotView && !isResolved && !isUpDown && !isStats5m && !isBacktest5m && activeView !== 'watchlist' && (
      ...
      {isFullscreen ? (
        ...
        <div style={{ flex: 1, overflow: 'hidden' }}>
          <UpDownView refreshSignal={upDownRefreshTick} />
        </div>
      ) : isStats5m ? (
        <div style={{ flex: 1, overflow: 'hidden' }}>
          <Polymarket5mStatsView key={statsRefreshTick} />
        </div>
      ) : isBacktest5m ? (
        <div style={{ flex: 1, overflow: 'hidden' }}>
          <Polymarket5mBacktestView key={backtestRefreshTick} />
        </div>
      ) : (
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          <div style={{ width: 360, flexShrink: 0, borderRight: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '5px 10px', borderBottom: `1px solid ${C.border}`, backgroundColor: C.header, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: 9, fontWeight: 'bold', color: activeView === 'watchlist' ? C.cyan : isResolved ? C.green : C.orange, textTransform: 'uppercase', letterSpacing: '0.5px', fontFamily: C.font }}>
                {activeView === 'watchlist' ? `WATCHLIST (${watchlistEntries.length})` : activeView === 'markets' ? `MARKETS (${markets.length})` : activeView === 'events' ? `EVENTS (${events.length})` : isResolved ? `RESOLVED EVENTS (${resolvedEvents.length})` : 'SPORTS'}
              </span>
            </div>
            {/* Scrollable list */}
      ...
      <TabFooter
        ...
        leftInfo={[
          ...
          { label: activeViewLabel, color: '#a3a3a3' },
          ...(selectedMarket && !isUpDown ? [{ label: selectedMarket.question?.slice(0, 40) + (selectedMarket.question?.length > 40 ? '…' : ''), color: '#6b7280' }] : []),
        ]}
