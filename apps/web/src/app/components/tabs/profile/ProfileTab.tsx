...
const LOCAL_PROFILE_CREATED_AT = new Date('2026-01-01T00:00:00.000Z').toISOString();
...
const ProfileTab: React.FC = () => {
  ...
  const fetchSectionData = useCallback(async () => {
    ...
    try {
      if (state.activeSection === 'usage') {
        ...
        const localUsage: UsageData = {
          period: 'lifetime',
          total_requests: session.requests_today || 0,
          total_credits_used: 0,
          current_balance: session.user_info?.credit_balance ?? session.credit_balance ?? 0,
          endpoint_usage: {},
        };
        dispatch({ type: 'SET_USAGE_DATA', data: localUsage });
      } else if (state.activeSection === 'billing') {
        if (!mountedRef.current || currentFetchId !== fetchIdRef.current) return;
        const localSubscription: AccountSubscriptionData = {
          user_id: 1,
          account_type: session?.user_info?.account_type || 'enterprise',
          credit_balance: session?.user_info?.credit_balance || session?.credit_balance || 0,
          credits_expire_at: null,
          support_type: 'community',
          last_credit_purchase_at: null,
          created_at: LOCAL_PROFILE_CREATED_AT,
        };
        dispatch({ type: 'SET_SUBSCRIPTION_DATA', data: localSubscription });
  ...
  const confirmRegenerateKey = useCallback(async () => {
    ...
    try {
      const newApiKey = `fk_local_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 12)}`;
...
const OverviewSection: React.FC<{ session: any; onLogout: () => void; isLoggingOut?: boolean }> = ({ session, onLogout, isLoggingOut }) => {
  const { refreshUserData, updateUserInfo } = useAuth();
  const accountType = session?.user_info?.account_type || 'free';
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      setSaveStatus({ type: 'success', text: 'Profile updated locally.' });
      await refreshUserData();
      setTimeout(() => { setSaveStatus(null); setEditing(false); }, 1200);
    } catch (err) {
      setSaveLoading(false);
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1 }}>
            ...
            <div>
              <div style={{ fontSize: '9px', fontWeight: 700, color: F.YELLOW, letterSpacing: '0.5px', marginBottom: '2px' }}>
                PROFILE INCOMPLETE ({percent}%) — Complete your local profile
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <div style={{ fontSize: '9px', color: F.MUTED, marginTop: '-8px', marginBottom: '12px' }}>
                  Stored locally only. Include country dial code if useful.
                </div>
              </div>
...
const SecuritySection: React.FC<{
  ...
  const handleEnableMFA = async () => {
    ...
    try {
      await updateUserInfo({ mfa_enabled: true });
      setMfaLoading(false);
      setStatusMessage({ type: 'success', text: 'MFA flag enabled locally.' });
      setTimeout(async () => { await refreshUserData(); setStatusMessage(null); }, 1200);
    } catch (err) {
      setMfaLoading(false);
  ...
  const confirmDisableMFA = async () => {
    ...
    try {
      await updateUserInfo({ mfa_enabled: false });
      setMfaLoading(false);
      setShowPasswordPrompt(false);
      setPassword('');
      setStatusMessage({ type: 'success', text: 'MFA flag disabled locally.' });
      setTimeout(async () => { await refreshUserData(); setStatusMessage(null); }, 1200);
    } catch (err) {
      setMfaLoading(false);
      setShowPasswordPrompt(false);
