...
// When no API key is set, core operations run locally (no API required).
...
async function apiCall<T = any>(path: string, body?: any): Promise<T> {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401) {
      throw new Error(
        _apiKey
          ? `QuantLib Curves API rejected the request (401). Check that your API key is valid.`
          : `QuantLib Curves API requires authentication. Sign in (or add an API key) in the QuantLib tab.`
      );
    }
...
export const curveApi = {
  build: (reference_date: string, tenors: number[], values: number[], curve_type = 'zero', interpolation = 'linear') =>
    useLocal() ? curvesLocal.build(reference_date, tenors, values, curve_type, interpolation) : apiCall('/quantlib/curves/build', { reference_date, tenors, values, curve_type, interpolation }),
  zeroRate: (reference_date: string, tenors: number[], values: number[], query_tenor: number, curve_type = 'zero', interpolation = 'linear') =>
    useLocal() ? curvesLocal.zeroRate(reference_date, tenors, values, query_tenor, curve_type, interpolation) : apiCall('/quantlib/curves/zero-rate', { reference_date, tenors, values, query_tenor, curve_type, interpolation }),
  discountFactor: (reference_date: string, tenors: number[], values: number[], query_tenor: number, curve_type = 'zero', interpolation = 'linear') =>
    useLocal() ? curvesLocal.discountFactor(reference_date, tenors, values, query_tenor, curve_type, interpolation) : apiCall('/quantlib/curves/discount-factor', { reference_date, tenors, values, query_tenor, curve_type, interpolation }),
  forwardRate: (reference_date: string, tenors: number[], values: number[], start_tenor: number, end_tenor: number, curve_type = 'zero', interpolation = 'linear') =>
    useLocal() ? curvesLocal.forwardRate(reference_date, tenors, values, start_tenor, end_tenor, curve_type, interpolation) : apiCall('/quantlib/curves/forward-rate', { reference_date, tenors, values, start_tenor, end_tenor, curve_type, interpolation }),
  instantaneousForward: (reference_date: string, tenors: number[], values: number[], query_tenor: number, curve_type = 'zero', interpolation = 'linear') =>
    useLocal() ? curvesLocal.instantaneousForward(reference_date, tenors, values, query_tenor, curve_type, interpolation) : apiCall('/quantlib/curves/instantaneous-forward', { reference_date, tenors, values, query_tenor, curve_type, interpolation }),
  curvePoints: (reference_date: string, tenors: number[], values: number[], query_tenors: number[], curve_type = 'zero', interpolation = 'linear') =>
    useLocal() ? curvesLocal.curvePoints(reference_date, tenors, values, query_tenors, curve_type, interpolation) : apiCall('/quantlib/curves/curve-points', { reference_date, tenors, values, query_tenors, curve_type, interpolation }),
};

// ── Interpolation (3) ──────────────────────────────────────────
export const interpolationApi = {
  interpolate: (x: number[], y: number[], query_x: number[], method = 'linear') =>
    useLocal() ? interpolationLocal.interpolate(x, y, query_x, method) : apiCall('/quantlib/curves/interpolate', { x, y, method, query_x }),
  derivative: (x: number[], y: number[], query_x: number, method = 'linear') =>
    useLocal() ? interpolationLocal.derivative(x, y, query_x, method) : apiCall('/quantlib/curves/interpolate-derivative', { x, y, method, query_x }),
  monotonicityCheck: (x: number[], y: number[], query_x: number[], method = 'linear') =>
    useLocal() ? interpolationLocal.monotonicityCheck(x, y, query_x, method) : apiCall('/quantlib/curves/monotonicity-check', { x, y, method, query_x }),
};
...
export const transformApi = {
  parallelShift: (reference_date: string, tenors: number[], values: number[], shift_bps: number, space = 'zero_rate') =>
    useLocal() ? transformLocal.parallelShift(reference_date, tenors, values, shift_bps, space) : apiCall('/quantlib/curves/parallel-shift', { reference_date, tenors, values, shift_bps, space }),
  ...
  scale: (reference_date: string, tenors: number[], values: number[], scale_factor: number) =>
    useLocal() ? transformLocal.scale(reference_date, tenors, values, scale_factor) : apiCall('/quantlib/curves/scale', { reference_date, tenors, values, scale_factor }),
};
...
export const constructionApi = {
  ...
  proxy: (reference_date: string, base_tenors: number[], base_values: number[], spread_bps: number) =>
    useLocal() ? constructionLocal.proxy(reference_date, base_tenors, base_values, spread_bps) : apiCall('/quantlib/curves/proxy', { reference_date, base_tenors, base_values, spread_bps }),
...
export const smoothApi = {
  constrainedFit: (tenors: number[], rates: number[]) =>
    useLocal() ? nelsonSiegelLocal.fit(tenors, rates) : apiCall('/quantlib/curves/constrained-fit', { tenors, rates }),
...
export const nelsonSiegelApi = {
  fit: (tenors: number[], rates: number[]) =>
    useLocal() ? nelsonSiegelLocal.fit(tenors, rates) : apiCall('/quantlib/curves/nelson-siegel/fit', { tenors, rates }),
  evaluate: (beta0: number, beta1: number, beta2: number, tau: number, query_tenors: number[]) =>
    useLocal() ? nelsonSiegelLocal.evaluate(beta0, beta1, beta2, tau, query_tenors) : apiCall('/quantlib/curves/nelson-siegel/evaluate', { beta0, beta1, beta2, tau, query_tenors }),
