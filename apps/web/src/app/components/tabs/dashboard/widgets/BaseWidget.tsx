...
// Map legacy i18n keys to display labels (when no locale file provides translation)
const WIDGET_KEY_LABELS: Record<string, string> = {
  'widgets.globalIndices': 'Global Indices',
  'widgets.forex': 'Forex',
  'widgets.commodities': 'Commodities',
  'widgets.crypto': 'Cryptocurrency',
  'widgets.marketNews': 'Market News',
  'widgets.economicIndicators': 'Economic Indicators',
  'widgets.geopoliticalRisk': 'Geopolitical Risk',
  'widgets.performanceTracker': 'Performance Tracker',
  'widgets.commodityGold': 'Commodity Gold',
  'widgets.news': 'News',
};

function displayTitle(title: string, t: (key: string) => string): string {
  if (!title || typeof title !== 'string') return title;
  const translated = t(title);
  if (translated !== title) return translated;
  // Only fix legacy i18n keys (widgets.xxx); leave normal titles unchanged to avoid extra spaces
  if (!title.startsWith('widgets.')) return title;
  const fallback = WIDGET_KEY_LABELS[title];
  if (fallback) return fallback;
  const afterDot = title.replace(/^widgets\./, '');
  const withSpaces = afterDot.replace(/([A-Z])/g, ' $1').replace(/^\s+/, '');
  return withSpaces ? withSpaces.charAt(0).toUpperCase() + withSpaces.slice(1) : title;
}
...
export const BaseWidget: React.FC<BaseWidgetProps> = ({
  ...
  const display = displayTitle(title, t);
