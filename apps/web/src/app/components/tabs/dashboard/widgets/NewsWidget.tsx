...
export const NewsWidget: React.FC<NewsWidgetProps> = ({
  ...
  // Filter and limit news based on category; ensure array
  const news = useMemo(() => {
    if (!allNews || !Array.isArray(allNews)) return [];
    const filtered = category === 'ALL'
      ? allNews
      : allNews.filter(a => a && a.category === category);
        ...
        {news.map((article, index) => (
          ...
          >
            ...
            <div style={{ color: 'var(--ft-color-text-muted)', fontSize: 'var(--ft-font-size-tiny)', lineHeight: '1.2' }}>
              {(article.summary && typeof article.summary === 'string' ? article.summary.substring(0, 100) : '')}
