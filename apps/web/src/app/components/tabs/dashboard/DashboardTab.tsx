...
import { DASHBOARD_TEMPLATES, TEMPLATE_PICKER_SEEN_KEY, SELECTED_TEMPLATE_KEY, CUSTOM_LAYOUTS_STORAGE_KEY, DashboardTemplate } from './dashboardTemplates';
...
const DashboardTab: React.FC<DashboardTabProps> = ({ onNavigateToTab }) => {
  ...
  const [customLayouts, setCustomLayouts] = useState<DashboardTemplate[]>([]);
  ...
  // Load custom layouts when opening template picker
  const loadCustomLayouts = useCallback(async () => {
    try {
      const raw = await getSetting(CUSTOM_LAYOUTS_STORAGE_KEY);
      if (!raw || !mountedRef.current) return;
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed) && mountedRef.current) setCustomLayouts(parsed);
    } catch {
      if (mountedRef.current) setCustomLayouts([]);
    }
  }, []);

  // Apply a dashboard template (built-in or custom)
  ...
  // Save current layout as a custom named template
  const saveAsCustomLayout = useCallback(async (name: string, description: string) => {
    ...
    const next = [...customLayouts, template];
    setCustomLayouts(next);
    try {
      await saveSetting(CUSTOM_LAYOUTS_STORAGE_KEY, JSON.stringify(next), 'dashboard');
      showSuccess(t('messages.layoutSaved') || 'Layout saved');
    } catch (err) {
      console.error('[DashboardTab] Save custom layout error:', err);
      showError('Failed to save layout');
    }
    setShowSaveLayoutModal(false);
  }, [customLayouts, state.widgets, t]);

  // Delete a custom layout
  const deleteCustomLayout = useCallback(async (id: string) => {
    const next = customLayouts.filter(l => l.id !== id);
    setCustomLayouts(next);
    try {
      await saveSetting(CUSTOM_LAYOUTS_STORAGE_KEY, JSON.stringify(next), 'dashboard');
    } catch (err) {
      console.error('[DashboardTab] Delete custom layout error:', err);
    }
  }, [customLayouts]);
  ...
  const renderWidget = (widget: WidgetInstance) => {
    ...
    switch (widget.type) {
      ...
      case 'polymarketwatchlist':
        return <PolymarketWatchlistWidget id={widget.id} onRemove={() => handleRemoveWidget(widget.id)} onNavigate={() => onNavigateToTab?.('polymarket')} />;
      case 'economic':
  ...
  return (
    <div style={{
      ...
      <div className="ft-dashboard" style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
            ...
            <button
              className="ft-toolbar-btn"
              onClick={() => { setIsFirstLaunch(false); setShowTemplatePicker(true); loadCustomLayouts(); }}
            ...
            <button
              ...
              title="Save current layout as a named template"
            >
              <Save size={11} />
              <span>SAVE AS</span>
            </button>
      ...
      <TemplatePicker
        ...
        onDeleteCustom={deleteCustomLayout}
        onClose={() => {
          setShowTemplatePicker(false);
          setIsFirstLaunch(false);
