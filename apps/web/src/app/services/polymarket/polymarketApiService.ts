...
const POLYMARKET_PROXY_PATH = '/api/v1/proxy/polymarket/gamma';

/** Default API origin when in dev so Polymarket works with or without Vite proxy (e.g. Tauri). */
const DEFAULT_DEV_API_ORIGIN = 'http://localhost:8000';

/** In browser, use backend proxy to avoid CORS. Override with VITE_POLYMARKET_GAMMA_PROXY or VITE_API_URL. */
function getGammaApiBase(): string {
  const env = typeof import.meta !== 'undefined' ? import.meta.env : undefined;
  const e = env as Record<string, string | boolean | undefined> | undefined;
  const explicit = e?.VITE_POLYMARKET_GAMMA_PROXY;
  if (explicit && typeof explicit === 'string') return (explicit as string).replace(/\/$/, '');
  const apiUrl = e?.VITE_API_URL;
  if (apiUrl && typeof apiUrl === 'string') return `${(apiUrl as string).replace(/\/$/, '')}${POLYMARKET_PROXY_PATH}`;
  // In dev, use absolute URL so Polymarket works in Tauri and when Vite proxy isn't used
  if (typeof window !== 'undefined' && e?.DEV) {
    return `${DEFAULT_DEV_API_ORIGIN}${POLYMARKET_PROXY_PATH}`;
  }
  if (typeof window !== 'undefined') return POLYMARKET_PROXY_PATH;
  return GAMMA_API_BASE;
}
...
class PolymarketServiceEnhanced {
  ...
  constructor() {
    this.gammaApiBase = getGammaApiBase();
  ...
  /** Fetch from Gamma API: try proxy first, then direct if proxy fails (e.g. API not running). */
  private async gammaFetch(pathSuffix: string, options?: RequestInit): Promise<Response> {
    const path = pathSuffix.startsWith('/') ? pathSuffix.slice(1) : pathSuffix;
    const urlProxy = `${this.gammaApiBase.replace(/\/$/, '')}/${path}`;
    try {
      const r = await fetch(urlProxy, options);
      if (r.ok) return r;
      if (r.status >= 500 || r.status === 404) {
        const urlDirect = `${GAMMA_API_BASE}/${path}`;
        const r2 = await fetch(urlDirect, options);
        return r2;
      }
      return r;
    } catch {
  ...
  async getMarkets(params?: MarketQueryParams): Promise<PolymarketMarket[]> {
    try {
      ...
      const response = await this.gammaFetch(path);
  ...
  async getMarketBySlug(slug: string): Promise<PolymarketMarket> {
    try {
      const response = await this.gammaFetch(`markets/slug/${slug}`);
  ...
  async getEventBySlug(slug: string): Promise<PolymarketEvent> {
    try {
      const response = await this.gammaFetch(`events/slug/${slug}`);
      if (!response.ok) throw new Error(`Gamma API error: ${response.status}`);
      return await response.json();
    } catch (error) {
  ...
  async getTags(): Promise<PolymarketTag[]> {
    try {
      const response = await this.gammaFetch('tags');
      if (!response.ok) throw new Error(`Gamma API error: ${response.status}`);
      return await response.json();
    } catch (error) {
  ...
  async getSports(): Promise<PolymarketSport[]> {
    try {
      const response = await this.gammaFetch('sports');
      if (!response.ok) throw new Error(`Gamma API error: ${response.status}`);
      return await response.json();
    } catch (error) {
  ...
  async search(query: string, type: 'markets' | 'events' | 'all' = 'all'): Promise<{ markets: PolymarketMarket[]; events: PolymarketEvent[] }> {
    try {
      ...
      const res = await this.gammaFetch(`public-search?${params.toString()}`);
